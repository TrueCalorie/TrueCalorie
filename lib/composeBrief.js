...
// Post-run briefs are for runs with meaningful volume. Shorter sessions and
// other sports get nothing (rest-day / before-training states cover them).
const MIN_POSTRUN_MOVING_MIN = 20

// The recovery window runs one hour from the END of the activity (start +
// elapsed), not from webhook receipt — Strava sync lag eats into the real
// window and the countdown should say so honestly.
...
// Returns { brief } | { deduped: true } | { skipped: reason }.
export async function composePostrunBrief({ userId, activity }) {
  ...
  // Gate 1: runs only
  if (sport !== 'running') return { skipped: 'not a run' }
  // Gate 2: meaningful volume
  if (movingMin < MIN_POSTRUN_MOVING_MIN) return { skipped: `under ${MIN_POSTRUN_MOVING_MIN} min` }

  const { settings, profile, checkins } = await loadUserContext(userId)
  if (!settings) throw new Error(`No user_settings for ${userId}`)

  // Gate 3: user opted out of post-run briefs (fuel_profiles.notify_postrun;
  // default true when there is no profile row or the column is null)
  if (profile?.notify_postrun === false) return { skipped: 'notify_postrun off' }
  ...
  // Window: one hour after the activity ENDED (start + elapsed; fall back to
  // moving time when elapsed is absent).
  const startMs      = new Date(activity.start_date).getTime()
  const durationSec  = activity.elapsed_time || activity.moving_time || 0
  const activityEnd  = Number.isFinite(startMs) ? startMs + durationSec * 1000 : Date.now()
  const windowEndsAt = new Date(activityEnd + RECOVERY_WINDOW_MIN * 60 * 1000)

  const userPrompt = `Write a post-workout recovery brief. The athlete's workout just synced from Strava.

Workout: ${activity.name || 'workout'} (${sport}), ${movingMin} min moving${miles ? `, ${miles} mi` : ''}${activity.average_heartrate ? `, avg HR ${Math.round(activity.average_heartrate)}` : ''}${activity.suffer_score ? `, relative effort ${Math.round(activity.suffer_score)}` : ''}.
...
${checkinSummary(checkins)}`
  ...
  return insertAndDeliver({
    ...
    body,
    macros:             { carbs_g, protein_g, window_min: RECOVERY_WINDOW_MIN },
