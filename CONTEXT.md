...
## 3. Current state (end of day, June 23, 2026)
...
20. **@capacitor/browser left in package.json deliberately** (unused after the 3.1.1 fix; coexists fine with @capacitor/inappbrowser; remove in 1.1).
...
4. **App Store submission: [REJECTED June 22 — build 35 bounced under Guideline 3.1.1. NEXT IS BUILD 36, the surgical code fix (Section 3 June 22 block), NOT a Resolution Center appeal.] The old "on a 3.1.1 bounce reply in Resolution Center citing the US-storefront basis, do not resubmit blind" plan is SUPERSEDED: it assumed a reflexive rejection, but the actual reviewer text NAMED the remedy (link out to the default browser), so a pure 3.1.1(a) appeal would lose. Correct move = ship the code fix + resubmit. As-submitted name/keywords/age-rating/privacy labels (June 14 block) still apply.**
   ...
   - **IMMEDIATE NEXT ACTION (build 36 rebuild):** MacinCloud build 36 from main (`npm install` pulls @capacitor/inappbrowser; build# > 35; **"App Store Connect" distribution NOT "TestFlight Internal Only"**; patch-plist.sh; re-stamp icons) -> device-verify BOTH the money round trip AND the new Safari return chain (pay in real Safari -> Return button -> app foregrounds -> Pro; repeat for cancel) -> submit build 36.
...
- Server: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, **STRIPE_PRICE_ID_MONTHLY (new)**, **STRIPE_PRICE_ID_ANNUAL (new)**, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, ANTHROPIC_API_KEY, STRAVA_CLIENT_ID, STRAVA_CLIENT_SECRET, PORTAL_RETURN_URL, **POSTHOG_KEY (new)**, **SENTRY_DSN (new June 20 PM; set in Vercel all envs incl. Preview; lib/sentry.js no-ops without it)**
