...
Last updated: June 17, 2026 (part 2; repo unchanged at 193f70c code on main; no new commits, strategy/operational state only). Part 1 covered marketing positioning, Isaiah content contractor, Mines coach call, and the shared individual-vs-organization developer-enrollment root cause on both app stores. Part 2 adds the Nutritionix paid-plan decision (do not buy pre-revenue), the plan to launch without Nutritionix (voice runs on Haiku; restaurant search shelved), the confirmed native architecture (bundled UI + remote backend), and a security review. 1.0 build 35 remains in App Review with manual release. This file is the single canonical copy of project state. Version history lives in git: `git log -- CONTEXT.md`. This file is updated only via the /wrap-session command; do not edit it ad hoc.
...
_Reframed June 17: items 1 and the new global cost-ceiling item below are PRE-LAUNCH seatbelts, not 1.1 features. Both gate real money flowing, so they ship before the store link goes live, not after._
...
- **Independent Contractor Agreement drafted:** Company owns work product; contractor warrants rights to footage he provides; 1099 if $600+/yr; Colorado law; 7-day termination. **Awaiting fill (date, Isaiah's legal name, Jackson's title) and signatures.**
