...
Last updated: June 14, 2026 (repo at 7ecfb7a on main; all session work merged and pushed). This file is the single canonical copy of project state. (Header reconciliation: the prior wrap recorded f9f563f, but the actual wrap commit that touched this file was f64ca95; corrected here.) Version history lives in git: `git log -- CONTEXT.md`. This file is updated only via the /wrap-session command; do not edit it ad hoc.
...
**Pending:** TestFlight confirmation the native icon processes and renders. The brand-mark background is swappable to #0a0a0a for an exact app-match, and green coverage is adjustable, if the device render warrants it.
