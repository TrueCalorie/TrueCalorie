...
## 3. Current state (end of day, June 22, 2026)

### Build 35 3.1.1 fix BUILT + grocery serving-context fix BUILT — both on isolated branches, NOT merged (June 22, 2026)

Two code branches were built this session. **The 3.1.1 fix is now MERGED to main (2a09aec) + pushed; the grocery fix is NOT merged and stays off main until build 36 is submitted.** The two branches were kept apart so the launch-critical resubmit (build 36) carries ONLY the 3.1.1 fix.
...
**(2) Grocery serving-context fix — branch `fix/grocery-serving-context`, commit 469569d (NEW, NOT merged, NOT pushed). Stays OFF main until build 36 is submitted.**
...
- **STILL OPEN (not addressed this session):** `LogFoodSheet.jsx` lines 207/236 force `verified:true` on ALL USDA results (`...usdaResults.map(f => ({ ...f, verified: true }))`), clobbering the per-item flag from usda.js, so Branded items are wrongly badged VERIFIED. Fold into the grocery branch before it merges. (Section 4.)
...
### GROCERY SERVING-CONTEXT FIX (BUILT June 22, ISOLATED — branch fix/grocery-serving-context, commit 469569d)

- Full details in the June 22 block of Section 3. **Does NOT merge to main until build 36 is submitted** (kept off the launch-critical resubmit path). Build clean; downstream consumers verified.
...
- **Fold in before this branch merges:** the `verified:true` override in LogFoodSheet.jsx (lines 207/236) that wrongly badges Branded USDA items VERIFIED (it clobbers the per-item flag now set correctly in usda.js). Not touched this session.
...
4. **App Store submission: [REJECTED June 22 — build 35 bounced under Guideline 3.1.1. NEXT IS BUILD 36, the surgical code fix (Section 3 June 22 block), NOT a Resolution Center appeal.] The old "on a 3.1.1 bounce reply in Resolution Center citing the US-storefront basis, do not resubmit blind" plan is SUPERSEDED: it assumed a reflexive rejection, but the actual reviewer text NAMED the remedy (link out to the default browser), so a pure 3.1.1(a) appeal would lose. Correct move = ship the code fix + resubmit. As-submitted name/keywords/age-rating/privacy labels (June 14 block) still apply.**
   ...
   - **Then:** the grocery branch (fix/grocery-serving-context, 469569d) may merge AFTER build 36 is submitted, not before (Section 3 / Section 4).
